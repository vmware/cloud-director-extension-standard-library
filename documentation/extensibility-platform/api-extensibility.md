...
The external system, as the name suggests, represents an external to Cloud Director system which will be doing the custom processing of API extensibility requests. The API filter is a Cloud Director entity which represents a rule for which requests should be handled by a given external system. To put it in short:
...
- urlMatcher - used to set a custom URL which when requested, will be processed by the external system. The urlMatcher consists of urlPattern and urlScope which together identify a URL which will be serviced by an external system:
    - External Endpoints - in the case of external endpoints, the url matcher will match requests under `/ext-api`, `/ext-ui/provider` and `/ext-ui/tenant/<tenant-name>`
        - urlScope - allowed values are `EXT_API`, `EXT_UI_PROVIDER`, `EXT_UI_TENANT` corresponding to `/ext-api`, `/ext-ui/provider`, `/ext-ui/tenant/<tenant-name>`
        - urlPattern - request URL pattern, written as a regular expression pattern. Element content cannot exceed 1024 characters. We expect the urlPattern to end with `.*` (it is like a prefix) which specifies that the part of the URL coming after will be redirected to an external endpoint. For example, if you want the external endpoint to service all requests matching `/ext-api/custom/.*`, the URL Matcher object should be:
        ```json
         {
            "urlPattern": "/custom/.*",
            "urlScope": "EXT_API"
        ...
        ```
        It is important to note that in the case of `TENANT` urlScope, the tenant name is not part of the urlPattern. The urlPattern will match the request after the tenant name - if request is `/ext-ui/tenant/testOrg/custom/test`, the pattern will match against `/custom/test`.
    - External Services - in the case of external services, the url matcher will match requests under `/api`
        - urlScope - only allowed value is `API`
        - urlPattern - request URL pattern, written as a regular expression pattern. Element content cannot exceed 1024 characters.
...
- `enabled` - true/false (a user provided field) - whether the external service is enabled or not
...
#### Enabled vs disabled external services
If the external service is not enabled, requests will not be routed to it for processing. Cloud Director acts as if the service does not exist. An external service must be disabled before being deleted.
...
2. __Authorization mechanism explained__
    ...
    But what does authorizing a service mean in this context? Authorizing a service means that upon making a request, the user security context will be verified against the request URI, request method and request content type based on the defined authorization rules (Resource Class Actions and ACL Rules).
    ...
    Each Resource Class Action contains a list of ACL Rules ([more info](#acl-rule)). ACL Rules are allow rules and hold the actual rules for which user/org/right has access to a resource (external resource or Cloud Director core resource).

    So in order to configure external service authorization, you need to defined at least one Resource Resource, one Resource Class Action and one ACL Rule.
...
Let's say we have an external service registeres and an API filter with the following urlPattern - `/api/org/.*/currentTime`. If we want a link to this custom URL to be returned each time a GET is executed on an organization, we must create a Link:
...
<ns2:ServiceLink xmlns="http://www.vmware.com/vcloud/v1.5"
    ...
    <ns2:ResourceType>application/vnd.vmware.admin.vcloud+xml</ns2:ResourceType>
</ns2:ServiceLink>
```
Response:
```xml
...
<Org xmlns="http://www.vmware.com/vcloud/v1.5" xmlns:rasd="http://schemas.dmtf.org/wbem/wscim/1/cim-schema/2/CIM_ResourceAllocationSettingData" xmlns:ovf="http://schemas.dmtf.org/ovf/envelope/1" xmlns:vssd="http://schemas.dmtf.org/wbem/wscim/1/cim-schema/2/CIM_VirtualSystemSettingData" xmlns:common="http://schemas.dmtf.org/wbem/wscim/1/common" xmlns:vmw="http://www.vmware.com/schema/ovf" xmlns:vmext="http://www.vmware.com/vcloud/extension/v1.5" xmlns:ovfenv="http://schemas.dmtf.org/ovf/environment/1" xmlns:ns9="http://www.vmware.com/vcloud/versions" name="STFTestOrg" id="urn:vcloud:org:5eac4ea6-11e4-4827-a249-ac8631779b92" href="https://localhost:8443/api/org/5eac4ea6-11e4-4827-a249-ac8631779b92" type="application/vnd.vmware.vcloud.org+xml">
    ...
    <Description>This is a test org created by STF Tests during setup and available for other tests to use.
Developers are free to use this org for routine purposes if they choose to use it</Description>
    <FullName>STF Test Org</FullName>
</Org>
```
...
A Resource Class entity represents an external entity type defined for an external service. Resource Classes are used by the Entity Resolver. The goal of the Entity Resolver is based on the URN identification of an entity to return URL to the entity's representation.

Part of the responsibility of the external services is to supply the necessary information in the Resource Class definition for the Entity Resolver functionality to work properly as part of their configuration process. Necessary information includes mime type, url template, nid and urn pattern. These properties should be supplied for every external service entity type (or resource class). They are part of registering external services resource classes to Cloud Director:
...
- __urlTemplate__ - String - the value of href attribute for resources of this class.  Supports following parameters:
    - `{baseUri}` - "Domain" part of the URL. Value set is vCloud Director REST API's baseUri.
...
{
    "externalSystem": {
        "id": "urn:vcloud:extensionEndpoint:vmware:test:1.0.0",
        "name": "test"
    },
    "urlMatcher": {
...
Shared subscriptions have been introduced as essential feature of the MQTT v5 protocol. In essence, this feature allows messages on a topic to be distributed/balanced among multiple clients, thereby improving the message troughput and fault tolerance in an MQTT system.
...
Shared subscriptions are useful particularly in high-scalability and failover scenarios:

* Scalability, having huge message flow that needs to be balanced among MQTT clients.
* Failover: The delivery semantics employ QoS 1 and 2 which ensures message delivery policy.

#### Other notes

Shared subscription can be used in hybrid scenarios, where some consumers may be grouped by a different group IDs and some consumers may not use shared subscription at all    
Consider the following example scenario: 
A producer sends 10 messages on a given topic. 
There are 6 subscribers listening on this topic configured as follows: 

| name | group  | messages received |
| ---- | ------ | ----------------- |
| sub1 | group1 | ~1/3 of messages  |
| sub2 | group1 | ~1/3 of messages  |
| sub3 | group1 | ~1/3 of messages  |
| sub4 | group2 | ~1/2 of messages  |
| sub5 | group2 | ~1/2 of messages  |
| sub6 |        | 100% of messages  |
...
### Building an API extension via HTTP (HTTP Proxy)
