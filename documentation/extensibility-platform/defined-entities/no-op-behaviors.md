...
## Behavior Definition
The no-op behavior execution type is ```noop```. By default no-op behavior execution always succeeds and returns a ```null``` result. However, you can configure the no-op behavior to return a custom result. Also you can configure no-op behaviors to always fail with custom error.

#### Special execution properties
There are some special properties which can be set in the behavior definition ```execution_properties```.
