...
## Prerequisites
In order to create a MQTT behavior you need a registered MQTT API extension in Cloud Director.
...
The `serviceId` from the `execution_properties` holds the id of the MQTT API extension which will be consuming the messages of this behavior. It is a required field.
...
Multiple task update responses can be sent back to Cloud Director (i.e. if you wish to update the task progress continuously). The last task update must complete the task (once the task is completed, later task updates regarding this task are ignored).
