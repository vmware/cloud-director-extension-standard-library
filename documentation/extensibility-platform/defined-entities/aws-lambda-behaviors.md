...
## Prerequisites
In order to create an AWS Lambda behavior in Cloud Diretor, you need to have created an function in [AWS Lambda](https://aws.amazon.com/lambda/) for the behavior to invoke.
...
```
The AWS Lambda behavior's `executionType` is `AWSLambdaFaaS`.  It is a required field.
...
The `aws_invocation_type` - `RequestResponse`/`Event`/`DryRun` - specifies the function invocation type. Possible values are `RequestResponse`, `Event` and `DryRun`. If no value is specified, `RequestResponse` is the default.
...
When invoking a AWS Lambda behavior the corresponding AWS Lambda function received the following event:
...
The `vcdContext` contains the VCD hostname and port, and an act-as token if one was requested (from the behavior definition).
...
```
The result of the behavior invocation is located in the `result` field of the associated `executeBehavior` task.
...
```
You can retrieve the execution log (if one was requiested with the `aws_include_log_tail` flag in the behavior definition) by using the `href` from the `resultReference`.
```
