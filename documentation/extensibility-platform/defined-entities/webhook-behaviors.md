# WebHook Behaviors
WebHook behaviors provide webHook functionality in the context of Cloud Director. In short, upon invocation a webHook behavior sends a POST request to a webHook server and after receiving the server's response, updates the behavior invocation task accordingly. The payload sent to the webHook server is configurable.

## Prerequisites
In order to create a webHook behavior you need to have a webHook server set-up to process requests from webHook behavior invocations. The webHook server must support HTTPs and its SSL certificate must be trusted in the organization of the user invoking the webHook behavior in order for the connection to be successful ([how to trust a certificate in Cloud Director](https://docs.vmware.com/en/VMware-Cloud-Director/10.4/VMware-Cloud-Director-Service-Provider-Admin-Portal-Guide/GUID-80B4CB1C-9353-4EB9-8557-4F6705949D0F.html)).

For testing purposes you can use [webhooks.site](https://webhook.site/) - it gives you a random URL which you can use as a webHook server URL. You can edit the response sent back from webhooks.site to each request. 
...
The ```_internal_key``` holds a shared secret string which is only known to Cloud Director and the webHook server. This shared secret will be used to generate the signature header of the request (part of the [HMAC authentication](#authentication-of-webhook-behavior-requests)). It is a required field.
...
The default payload sent to webHook servers (when there is no template set) is a JSON containing information about the actual behavior invocation (invocation arguments, behavior information, entity information and some metadata).
...
You can also customize this payload by setting a template in the behavior definition. <a href="https://freemarker.apache.org/docs/dgui_quickstart_template.html" target="_blank">FreeMarker template engine</a> is used to render the payload from the provided template and the data model.
#### The Data Model
...
- `entity_string` - a json format string of the entity contents
...
Slack has a functionality of creating incoming webhooks which can be used for posting messages to slack channels. Webhook behaviors can be used to integrate slack incoming webhooks with Cloud Director, allowing Cloud Director to post messages directly to a slack channel. 
